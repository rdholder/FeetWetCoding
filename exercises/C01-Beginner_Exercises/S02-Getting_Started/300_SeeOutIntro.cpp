...
int SeeOutIntro::runExercise()  // copy the exercise name & paste it into main.cpp
{
    ...
    // "seeout" is a FeetWetCoding tool designed to
    // work like std::cout, except that it prints its
    // output right into to your working window so you
    // don't have to go looking for it in the QtCreator
    // IDE (we'll learn how to do that a little later).

    // The next few lines demonstrate how "seeout" works,
    // but it's not setup exactly right, as you'll
    // see when you run the exercise and compare the
    // Exercise output to the Solution output.

    // Can you make these lines match the solution?

    seeout << " 1" << "\n";   // the "\n" mean move down to the next line
    seeout << " 2" << "\n";
    seeout << " 3" << "\n";
    seeout << " 4" << "\n";
    seeout << " 5" << "\n";
